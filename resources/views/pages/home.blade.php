...
@section('content')
    @php
        ...
        $heroSlides = [['image' => 'images/hero/hero-1.jpg'], ['image' => 'images/hero/hero-2.jpg']];
    ...
    <section id="home" x-data="heroSlider({{ count($heroSlides) }})" x-on:mouseenter="stopAutoplay()"
        x-on:mouseleave="startAutoplay(); resetPointer()" x-on:mousemove="handlePointerMove($event)"
        class="hero-stage relative isolate h-[70vh] overflow-hidden md:h-[90vh]">
        @foreach ($heroSlides as $index => $slide)
            <div x-bind:class="active === {{ $index }} ? 'opacity-100' : 'opacity-0'"
                class="hero-slide absolute inset-0 z-0 transition-opacity duration-1000 ease-out"
                style="will-change: opacity;" x-bind:aria-hidden="active === {{ $index }} ? 'false' : 'true'">
                <div class="hero-slide-bg" x-bind:style="getLayerStyle({{ $index }}, 'back')">
                    <img src="{{ asset($slide['image']) }}" alt="" width="1920" height="1080"
            ...
            <div class="hero-copy mx-auto max-w-4xl text-center text-white" x-bind:style="getContentStyle()">
                @foreach ($heroSlides as $index => $slide)
                    <div x-show="active === {{ $index }}" x-cloak
                        ...
                        x-transition:leave-end="-translate-y-2 opacity-0" class="mx-auto"
                        style="will-change: transform, opacity;">
                        <div class="mb-5 flex flex-wrap items-center justify-center gap-2">
                            <span
                                class="inline-flex min-h-8 items-center gap-2 rounded-full border border-white/18 bg-white/10 px-3.5 py-1 text-xs font-semibold tracking-[0.12em] text-white/90 shadow-[0_16px_38px_-28px_rgba(15,23,42,0.45)]"
                        ...
                        <h1
                            class="text-balance text-4xl font-black leading-[1.05] tracking-tight text-white sm:text-3xl md:text-4xl lg:text-5xl">
                            {{ __('site.hero.title') }}
                        </h1>
                        ...
                        <div class="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
                            <a href="https://ec.alkayantech.com/" target="_blank" rel="noopener noreferrer"
                                onclick="trackEvent('website_click', { location: 'hero' })"
                                class="btn-base btn-primary min-h-[52px] w-full sm:min-w-[11rem] sm:w-auto px-8">
                                {{ __('Visit our website') }}
                            </a>
                            <a href="https://wa.me/{{ $whatsappNumber }}?text={{ $whatsappGreeting }}" target="_blank"
                                rel="noopener noreferrer" onclick="trackEvent('whatsapp_click', { location: 'hero' })"
                                class="btn-base min-h-[52px] w-full sm:min-w-[11rem] sm:w-auto border border-white/18 bg-white/10 px-8 text-white shadow-[0_14px_30px_-22px_rgba(15,23,42,0.6)] transition duration-300 hover:-translate-y-0.5 hover:border-white/28 hover:bg-white/14">
                ...
                <div class="mt-8 flex items-center justify-center gap-2">
                    @foreach ($heroSlides as $index => $slide)
                        <button type="button" x-on:click="goToSlide({{ $index }})"
                            ...
                            aria-label="الانتقال إلى الشريحة {{ $index + 1 }}"></button>
                    @endforeach
                </div>
            </div>
        </div>
    ...
    <section id="products" class="section-shell bg-white dark:bg-gray-900">
        <div class="container-custom">
            <div data-motion-group class="mx-auto mb-12 max-w-2xl text-center md:mb-16">
                <h2 data-motion-item data-motion-order="0"
                    class="text-3xl font-bold leading-tight tracking-tight text-gray-900 dark:text-white md:text-4xl">
            ...
            @if ($products->isNotEmpty())
                <div x-data="productCinemaSlider({{ $products->count() }})"
                    ...
                    <div
                        class="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/5 via-transparent to-black/30">
                    </div>
                    ...
                    @foreach ($products as $index => $product)
                        ...
                        <article x-bind:style="getSlideStyle({{ $index }})"
                            ...
                            <div
                                ...
                                <div x-bind:style="getMediaStyle({{ $index }})"
                                    ...
                                    <div
                                        class="absolute inset-0 bg-gradient-to-l from-transparent via-black/10 to-black/50 lg:bg-gradient-to-r lg:from-transparent lg:via-black/10 lg:to-black/55">
                                    </div>
                                </div>

                                <div class="order-2 flex h-full items-center lg:order-1">
                                    <div x-bind:style="getTextStyle({{ $index }})"
                                        ...
                                        <span
                                            class="inline-flex min-h-8 items-center rounded-full border border-white/15 bg-white/6 px-3 py-1 text-xs font-semibold tracking-[0.12em] text-orange-200">
                                            {{ __('site.products.featured') }}
                                        </span>
                                        <h3
                                            class="mt-5 max-w-xl text-3xl font-black leading-tight text-white md:text-4xl lg:text-5xl">
                                            {{ $product->name }}
                                        </h3>
                                        ...
                                        <div
                                            class="mt-8 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center {{ $productActionsAlign }}">
                                            <a href="https://wa.me/{{ $whatsappNumber }}?text={{ $productInquiryMessage }}"
                        ...
                        <div
                            ...
                            @foreach ($products as $index => $product)
                                <button type="button" x-on:click="goToSlide({{ $index }})"
                                    ...
                                    class="h-3 rounded-full transition-[width,background-color] duration-300"
                                    aria-label="{{ __('site.products.go_to', ['number' => $index + 1]) }}"></button>
                            @endforeach
                        </div>
                    @endif
                </div>
    ...
    @if ($trustContent['is_enabled'])
        <x-sections.trust-conversion :content="$trustContent" :whatsapp-number="$whatsappNumber" />
    @endif

    {{-- Contact CTA: real lead capture with WhatsApp and form --}}
    <section id="contact" x-data="contactForm({
        ...
        <div class="container-custom">
            <div x-cloak x-show="toastVisible" x-transition:enter="transition duration-300 ease-out"
            ...
            <div data-animate class="hidden-anim transform-gpu contact-panel mx-auto max-w-5xl text-center"
                data-motion-group>
                ...
                <p data-motion-item data-motion-order="2"
                    class="mx-auto mt-5 max-w-2xl text-base leading-8 text-[var(--text-secondary)] md:text-lg">
                    {{ __('site.contact.description') }}
                </p>

                <div data-motion-item data-motion-order="3"
                    class="mt-10 flex flex-wrap items-center justify-center gap-4">
                    <a href="https://wa.me/{{ $whatsappNumber }}?text={{ $whatsappGreeting }}" target="_blank"
                        rel="noopener noreferrer" onclick="trackEvent('whatsapp_click', { location: 'contact_section' })"
                        class="btn-base btn-primary min-h-[52px] min-w-[11rem] px-8">
                        {{ __('site.contact.whatsapp') }}
                    </a>
                    <a href="tel:+{{ config('services.whatsapp.number') }}"
                        class="btn-base btn-secondary min-h-[52px] min-w-[11rem] px-8">
                ...
                @if (session('success'))
                    <div
                        class="mx-auto mt-6 max-w-2xl rounded-2xl border border-emerald-400/25 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-200">
                        {{ session('success') }}
                    </div>
                @endif

                <div data-motion-item data-motion-order="5" class="mx-auto mt-10 max-w-2xl text-center">
                    <form id="contactForm" x-ref="form" x-on:submit.prevent="submit"
                        action="{{ route('contact.store', ['locale' => $locale]) }}" method="POST"
                        class="contact-form space-y-5 p-6 sm:p-8">
                        @csrf

                        <div x-show="errorMessage" x-transition.opacity
                            class="rounded-xl border border-red-300/30 bg-red-50 px-4 py-3 text-sm font-medium text-red-700 dark:border-red-400/20 dark:bg-red-500/10 dark:text-red-200">
                        ...
                        <div class="space-y-2 text-start">
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name') }}"
                                class="contact-input" placeholder="{{ __('site.contact.form.name_placeholder') }}"
                                required>
                            <p x-show="errors.name" class="text-xs text-red-600 dark:text-red-300"
                        ...
                        <div class="space-y-2 text-start">
                            ...
                            <input id="phone" name="phone" type="text" inputmode="numeric" autocomplete="tel"
                                maxlength="10" pattern="05[0-9]{8}" title="{{ __('site.contact.form.phone_invalid') }}"
                                value="{{ old('phone') }}" class="contact-input"
                                placeholder="{{ __('site.contact.form.phone_placeholder') }}" required>
                            <p x-show="errors.phone" class="text-xs text-red-600 dark:text-red-300"
                        ...
                        <div class="space-y-2 text-start">
                            ...
                            <textarea id="message" name="message" rows="5" class="contact-input"
                                placeholder="{{ __('site.contact.form.message_placeholder') }}" required>{{ old('message') }}</textarea>
                            <p x-show="errors.message" class="text-xs text-red-600 dark:text-red-300"
                                x-text="errors.message?.[0]"></p>
                            @error('message')
                                <p class="text-xs text-red-600 dark:text-red-300">{{ $message }}</p>
                            @enderror
                        </div>
