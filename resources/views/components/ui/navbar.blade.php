...
<header x-data="premiumNavbar()" x-init="init()" @keydown.escape.window="closeMobileMenu()"
    @scroll.window.throttle.50ms="updateScrolledState()"
    class="sticky top-0 z-50 border-b transition-all duration-300 ease-in-out" :class="isScrolled
    ...
    <nav class="container-custom flex items-center gap-10 transition-all duration-300 ease-in-out"
        ...
        <a href="{{ $homeUrl }}" class="inline-flex shrink-0 items-center"
            ...
            <div
                class="px-2 py-1 rounded-lg bg-white/90 dark:bg-white/10 transition-all duration-300 dark:shadow-[0_0_15px_rgba(255,115,0,0.25)]">
        ...
        <ul class="hidden flex-1 items-center justify-end gap-10 md:flex">
            <li>
                <a href="{{ $homeUrl }}" @click="setActiveLink('home')"
        ...
        <div class="ms-auto flex items-center gap-2">
            <a href="{{ $switchUrl }}" onclick="this.href += window.location.hash || ''"
            ...
            <button onclick="toggleDark()" type="button"
                ...
                <svg class="absolute w-5 h-5 text-yellow-500 transition-all duration-500 opacity-100 scale-100 rotate-0 dark:opacity-0 dark:scale-0 dark:-rotate-90"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M7.05 16.95l-1.414 1.414M16.95 16.95l1.414 1.414M7.05 7.05 5.636 5.636" />
                </svg>
            ...
            <button @click="toggleMobileMenu()" type="button"
                ...
                <svg x-show="isMobileMenuOpen" x-cloak class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
    ...
    <div x-show="isMobileMenuOpen" x-cloak @click="closeMobileMenu()"
        x-transition:enter="transition-all duration-300 ease-in-out" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition-all duration-300 ease-in-out"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        class="fixed inset-0 z-40 bg-black/40 md:hidden"></div>

    <div x-show="isMobileMenuOpen" x-cloak @click.stop x-transition:enter="transition-all duration-300 ease-in-out"
        ...
        <div class="rounded-xl border border-gray-100 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
            <ul class="divide-y divide-gray-100 dark:divide-gray-800">
                <li>
                    <a href="{{ $homeUrl }}" @click="setActiveLink('home'); closeMobileMenu()"
                ...
                <li>
                    <a href="#about" @click="setActiveLink('about'); closeMobileMenu()"
                        class="block px-4 py-3 text-sm font-medium text-gray-600 transition-all duration-300 hover:bg-gray-50 hover:text-orange-500 dark:text-gray-300 dark:hover:bg-gray-800/80"
                ...
                <li>
                    <a href="#services" @click="setActiveLink('services'); closeMobileMenu()"
                        class="block px-4 py-3 text-sm font-medium text-gray-600 transition-all duration-300 hover:bg-gray-50 hover:text-orange-500 dark:text-gray-300 dark:hover:bg-gray-800/80"
